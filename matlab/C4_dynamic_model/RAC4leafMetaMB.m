...
if any(isnan(LM_con)) || any(LM_con>1e10)
    % In some cases, the solver returns a vector of NaN, which causes
    % errors later on. To prevent this, a vector of NaN is returned.
    % In some other cases, the solver returns very high values for MC_CO2,
    % which causes Gs and Gs0 to be NaN and results in an error, since vgs
    % will not be set.
    LM_DYDT = nan(size(LM_con));
    return
end
